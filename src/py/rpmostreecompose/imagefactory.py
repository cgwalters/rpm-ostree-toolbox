...
from .utils import run_sync, fail_msg, TrivialHTTP, log, checkOSTree
...
class AbstractImageFactoryTask(ImageTaskBase):
    ...
    def formatKS(self, ksfile):
        ...
        if not checkOSTree(ostree_location):
            fail_msg("Unable to verify find {0}/config {1} and verify it as an ostree repository.  Please check your repository and re-run".format(ostree_location, local_path))
        else:
            log("Verified OSTree repository: {0}".format(ostree_location))
